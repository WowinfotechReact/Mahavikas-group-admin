...
import MasterGovPortalDataModal from './MasterGovPortalDataModal';
import { MasterStatGetStateList, ChangeStateStatus } from 'services/Master Crud/MasterGovPortalApi';

const MasterGovPortalData = () => {
  ...
  const [portalListData, setPortalListData] = useState([]);
  const [openMasterGovPortalModal, setOpenMasterGovPortalModal] = useState(false);
      ...
      if (data) {
        if (data?.data?.statusCode === 200) {
          ...
          if (data?.data?.responseData?.data) {
            ...
            setPortalListData(MasterStateListData);
          }
        } else {
  ...
  const addMasterGovPortalBtnClick = () => {
    setModelRequestData({
      ...modelRequestData,
      Action: null,
      governmentPortalKeyID: null,
      userKeyID: null
    });
    setOpenMasterGovPortalModal(true);
  };
  const EditMasterGovPortalBtnClick = (row) => {
    setModelRequestData({
      ...modelRequestData,
      Action: 'Update',
      govtPortalCategoryID: row.govtPortalCategoryID
    });
    setOpenMasterGovPortalModal(true);
  };
  ...
  const confirmStatusChange = async (row, user) => {
    try {
      const { governmentPortalKeyID } = row; // Destructure to access only what's needed
      const response = await ChangeStateStatus(governmentPortalKeyID, user.userKeyID);
      ...
      if (response && response.data.statusCode === 200) {
        ...
        setModelAction('Government portal status changed successfully.');
      } else {
      ...
      console.error('Error changing Government portal status:', error);
      setShowSuccessModal(true);
  ...
  return (
    <>
      <div className="card w-full max-w-[50vh] mx-auto h-auto">
        <div className="card-body p-2 bg-white shadow-md rounded-lg">
          ...
          <div className="d-flex justify-content-between align-items-center mb-1">
            <h5 className="m-0">Government Portal Data </h5>
            <button onClick={() => addMasterGovPortalBtnClick()} className="btn btn-primary btn-sm d-inline d-sm-none">
          ...
          <div className="d-flex justify-content-between align-items-center mb-1">
            ...
            />
            <Tooltip title="Add Gov Portal">
              <button onClick={() => addMasterGovPortalBtnClick()} className="btn btn-primary btn-sm d-none d-sm-inline">
                <i className="fa-solid fa-plus" style={{ fontSize: '11px' }}></i>
                <span className="d-none d-sm-inline"> Add Gov Portal</span>
              </button>
            </Tooltip>
          </div>
              ...
              <tbody>
                {portalListData?.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="text-center">
                      <Tooltip title="Update Gov Portal Data">
                        <button
                          style={{
                          ...
                          onClick={() => EditMasterGovPortalBtnClick(row)}
                          type="button"
      ...
      {openMasterGovPortalModal && (
        <MasterGovPortalDataModal
          show={openMasterGovPortalModal}
          onHide={() => setOpenMasterGovPortalModal(false)}
...
export default MasterGovPortalData;
