...
import { ChangeLeadFollowUpStatus, GetFollowUpTypeLookupList } from 'services/LeadAPI/FollowUpAPI';
const FollowUpStageModal = ({ show, onHide, setIsAddUpdateActionDone, modalRequestData }) => {
    ...
    const AssignServiceCallToSiteEngData = async (apiParam) => {
        setLoader(true)
        try {
            let url = '/ChangeLeadFollowUpStatus'; // Default URL for Adding Data

            const response = await ChangeLeadFollowUpStatus(url, apiParam);
            if (response) {
                if (response?.data?.statusCode === 200) {
                    setLoader(false)
                    setShowSuccessModal(true);
                    setModelAction(
                        'Follow up stages changed successfully!'
